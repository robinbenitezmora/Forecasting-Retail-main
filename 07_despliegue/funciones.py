...
# FUNCION DE CREACION DE VARIABLES
def crear_variables(x):
    ...
    x["rotura_stock_3"] = (
        ...
        .ventas.transform(lambda x: rotura_stock(x, 3))
        .values
    )
    x["rotura_stock_7"] = (
        x.groupby(["store_id", "item_id"])
        .ventas.transform(lambda x: rotura_stock(x, 7))
        .values
    )
    x["rotura_stock_15"] = (
        x.groupby(["store_id", "item_id"])
        .ventas.transform(lambda x: rotura_stock(x, 15))
        .values
    )
    ...
    def min_movil(x, variable, num_periodos=7):
        ...
        for cada in range(2, num_periodos + 1):
            minm[variable + "_minm_" + str(cada)] = (
                x[variable].shift(1).rolling(cada).min()
            )
        return minm
    ...
    def media_movil(x, variable, num_periodos=7):
        mm = pd.DataFrame()
        for cada in range(2, num_periodos + 1):
            mm[variable + "_mm_" + str(cada)] = (
                x[variable].shift(1).rolling(cada).mean()
            )
        return mm
    ...
    def max_movil(x, variable, num_periodos=7):
        maxm = pd.DataFrame()
        for cada in range(2, num_periodos + 1):
            maxm[variable + "_maxm_" + str(cada)] = (
                x[variable].shift(1).rolling(cada).max()
            )
        return maxm
    ...
    x_unido.drop(
        columns=["sell_price", "rotura_stock_3", "rotura_stock_7", "rotura_stock_15"],
        inplace=True,
    )

    # Crear una sola variable para el producto
    x_unido.insert(
        loc=0, column="producto", value=x_unido.store_id + "_" + x_unido.item_id
    )
...
def transformar_variables(x, y=None, modo="entrenamiento"):
    ...
    ruta_ohe = "./ohe_retail.pkl"
    ruta_te = "./te_retail.pkl"
...
# FUNCION DE PRESELECCION DE VARIABLES
...
def lanzar_entrenamiento(df):
    ...
    ruta_modelos = "./lista_modelos_retail.pkl"
...
def lanzar_ejecucion(df):
    ...
    Que tiene que tener la estructura del fichero DatosParaProduccion.csv de la carpeta 07_Despliegue.
...
def forecast_recursivo(x):
    ...
    Que tiene que tener la estructura del fichero DatosParaProduccion.csv
